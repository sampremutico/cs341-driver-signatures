...
class SimpleCNN(torch.nn.Module):
	def __init__(self, input_size, hidden_size, num_classes=2, num_layers=1):
		...
		self.conv2 = nn.Conv1d(hidden_size[0], hidden_size[1], kernel_size=3, padding=1)
	...
	def forward(self, x):
		...
		x_pool_1 = self.pool_1(x_conv_1_out)
		print("Pool1 : {}".format(x_pool_1.shape))
		x_layer_1_out = self.relu(x_pool_1)

		x_conv_2_out = self.conv2(x_layer_1_out)
		print("Conv2 : {}".format(x_conv_2_out.shape))
		x_pool_2 = self.pool_2(x_conv_2_out)
